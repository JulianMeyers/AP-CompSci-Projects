...
public class MinePanel extends JPanel {
	...
	public void checkForZeroes(int x, int y)
	{
	    ...
	    if (isLegal(x,y)) {
            ...
            if (mySquares[x][y].getNeighboringMines() == 0) {
                checkIfLegal(x-1, y-1);
                checkIfLegal(x-1, y);
                checkIfLegal(x-1, y+1);
                checkIfLegal(x, y-1);
                //checkForZeroes(x, y); this is the square, no need to check it again
                checkIfLegal(x, y+1);
                checkIfLegal(x+1, y-1);
                checkIfLegal(x+1, y);
                checkIfLegal(x+1, y+1);
	...
	public boolean isLegal(int x, int y) {
        if (x < numCellsAcross && x >= 0 && y < numCellsDown && y >= 0 ) {
            return true;
        } else {
            return false;
        }
	...
	public void checkIfLegal(int x, int y) {
        if (isLegal(x,y)) {
            if (mySquares[x][y].getMyStatus() == MineStatus.ORIGINAL) {
                checkForZeroes(x,y);
            }
        }
    }
